...
@section('content')
            ...
            <div class="card-body">
              ...
              @if($pendingFilesForUser > 0)
                <div class="alert alert-warning alert-dismissible fade show mb-3" role="alert">
                  <h6 class="alert-heading mb-1">
                    ...
                    {{ $pendingFilesForUser == 1 ? 'file' : 'files' }} to review!
                  </h6>
              ...
              @if(config('app.debug'))
                <div class="alert alert-info small mb-2">
                  ...
                  Role: {{ auth()->user()->role->name ?? 'No Role' }}<br>
                  Has view_files: {{ hasPermission('view_files') ? 'YES' : 'NO' }}<br>
                  Has create_files: {{ hasPermission('create_files') ? 'YES' : 'NO' }}<br>
                  Permissions:
                  {{ auth()->user()->role ? auth()->user()->role->permissions->pluck('name')->implode(', ') : 'None' }}
                </div>
              @endif
              <div class="d-flex flex-column flex-sm-row gap-2">
                ...
                @if(hasPermission('create_files'))
                  <a href="{{ route('files.create') }}" class="btn btn-primary btn-lg w-100 flex-grow-0"
                    title="Create a New File">
                    <i class="bx bx-plus me-1"></i> Add New File
                  </a>
                @endif
                @if(hasPermission('view_files'))
                  <button type="button" class="btn btn-success btn-lg w-100 flex-grow-0" onclick="openQrScanner()"
          ...
          <div class="col-12 col-sm-5 order-1 order-sm-2 text-center text-sm-left">
            <div class="card-body pb-0 px-0 px-md-4 d-flex justify-content-center justify-content-sm-start">
              <img src="{{asset('public/assets/img/illustrations/man-with-laptop-light.png')}}" height="140"
                alt="View Badge User" class="img-fluid" style="max-height: 140px;"
            ...
            <div class="card-body">
              <div class="card-title d-flex align-items-start justify-content-between">
                <div class="avatar flex-shrink-0">
                  <img src="{{asset('public/assets/img/icons/unicons/chart-success.png')}}" alt="chart success"
                    class="rounded">
                </div>
              </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3 mb-4 mb-sm-0">
          <div class="card h-100">
            <div class="card-body">
              <div class="card-title d-flex align-items-start justify-content-between">
                <div class="avatar flex-shrink-0">
                  <img src="{{asset('public/assets/img/icons/unicons/wallet-info.png')}}" alt="Credit Card"
                    class="rounded">
                </div>
              </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3 mb-4 mb-sm-0">
          <div class="card h-100">
            <div class="card-body">
              <div class="card-title d-flex align-items-start justify-content-between">
                <div class="avatar flex-shrink-0">
                  <img src="{{asset('public/assets/img/icons/unicons/cc-primary.png')}}" alt="Credit Card"
                    class="rounded">
                </div>
              </div>
  ...
  <script>
    ...
    // Define QR Scanner functions immediately to prevent "not defined" errors
    // These will be available globally as soon as this script loads
          ...
          .then(response => {
            if (!response.ok) {
              return response.json().then(errData => {
                console.error('QR Scan error Dashboard:', errData);
          ...
          .then(data => {
            ...
            if (data.success || data.message) {
              ...
              if (data.can_edit_status && data.file_id) {
                ...
                updateScanningStatus('<span class="text-success"><i class="bx bx-check-circle me-1"></i> ' + message + '</span>');
                    ...
                    Swal.fire({
                      ...
                      if (result.isConfirmed) {
                        ...
                        fetch('{{ route("file-statuses.update") }}', {
                          ...
                          // User chose not to close, just redirect
                          window.location.href = '/files';
                        }
                      });
                    } else {
                      // No SweetAlert, just redirect
                      window.location.href = '/files';
                    }
                  }, 500);
                } else {
                  // User cannot edit status, just show success and redirect
