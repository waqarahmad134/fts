      ...
      <form id="permissionsForm">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="permissionsModalLabel">Manage Permissions for Role: <span
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          Object.keys(data.permissions).forEach(function (group) {
            const groupName = group; // Already user-friendly: Files, Users, Profile, File Status
            html += `
            ...
            data.permissions[group].forEach(function (perm) {
              ...
              const displayName = perm.description || perm.name; // Use description if available
              html += `
                <div class="col-6 col-md-4">
                  <div class="form-check">
                    <input class="form-check-input group-${group}" type="checkbox" name="permissions[]" value="${perm.id}" id="perm_${perm.id}" ${checked}>
                    <label class="form-check-label" for="perm_${perm.id}">${displayName}</label>
                  </div>
                </div>`;
            });
